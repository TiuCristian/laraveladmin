...
<head>
  ...
  <title>Pages | CMS Admin Panel</title>
...
<body>
    ...
    <main class="app-wrapper">
      <div class="container-fluid">
            ...
            <div class="row">
              <div class="col-md-5 mb-3 mb-md-0">
                ...
                <div class="d-flex flex-wrap gap-3 small">
                  <div class="form-check"><input class="form-check-input col-toggle" type="checkbox" id="colAuthor" data-col="col-author" checked><label class="form-check-label" for="colAuthor">Author</label></div>
        ...
        <div class="app-page-head d-flex align-items-center justify-content-between mb-4">
          <div>
            <h3 class="mb-0 d-inline-block me-2">Pages</h3>
            <a href="{{ route('pages.create') }}" class="btn btn-outline-primary btn-sm mb-1">Add New</a>
        ...
        <div class="d-flex flex-wrap align-items-center justify-content-between mb-2">
          <ul class="list-inline mb-2 mb-md-0 small">
            <li class="list-inline-item"><a href="{{ route('pages.index', ['filter' => 'all']) }}" class="{{ $filter === 'all' ? 'fw-bold text-body text-decoration-none' : 'text-primary text-decoration-none hover-primary' }}">All <span class="text-muted fw-normal">({{ $counts['all'] }})</span></a> <span class="text-muted mx-1">|</span></li>
            <li class="list-inline-item"><a href="{{ route('pages.index', ['filter' => 'published']) }}" class="{{ $filter === 'published' ? 'fw-bold text-body text-decoration-none' : 'text-primary text-decoration-none hover-primary' }}">Published <span class="text-muted">({{ $counts['published'] }})</span></a> <span class="text-muted mx-1">|</span></li>
            <li class="list-inline-item"><a href="{{ route('pages.index', ['filter' => 'pillar']) }}" class="{{ $filter === 'pillar' ? 'fw-bold text-body text-decoration-none' : 'text-primary text-decoration-none hover-primary' }}">Pillar Content <span class="text-muted">({{ $counts['pillar'] }})</span></a> <span class="text-muted mx-1">|</span></li>
        ...
        <div class="card border-0 shadow-sm mb-4">
              ...
              <thead class="table-light">
                <tr>
                  <th scope="col" style="width: 40px;" class="ps-3 border-bottom-0">
                    <input class="form-check-input" type="checkbox" id="selectAll" onclick="document.querySelectorAll('input[name=\'page_ids[]\']').forEach(cb => cb.checked = this.checked)">
                  </th>
                  <th scope="col" class="border-bottom-0 text-primary fw-medium">Title <i class="fi fi-rr-caret-up ms-1 small"></i></th>
                  <th scope="col" class="border-bottom-0 text-body fw-medium col-author">Author</th>
              ...
              <tbody>
                ...
                <tr id="row{{ $page->id }}">
                  <td class="ps-3 align-top pt-3">
                    <input class="form-check-input" type="checkbox" name="page_ids[]" value="{{ $page->id }}" form="bulkForm">
                  </td>
                  <td class="align-top pt-3" style="width: 35%;">
                    ...
                    <div class="small mt-1 text-muted" style="font-size: 0.8rem;">
                      @if($page->trashed())
                        ...
                        <form action="{{ route('pages.forceDelete', $page->id) }}" method="POST" class="d-inline">
                          @csrf
                          @method('DELETE')
                          <button type="submit" class="btn btn-link p-0 m-0 align-baseline text-decoration-none text-danger hover-danger" style="font-size: 0.8rem;" onclick="return confirm('Are you sure you want to permanently delete this page?')">Delete Permanently</button>
                        </form>
                      @else
                  ...
                  <td class="text-center align-top pt-3 col-author">
                  ...
                  <td class="align-top pt-3 col-date" style="width: 15%;">
                    ...
                    <span class="text-muted small">{{ $page->created_at->format('Y/m/d  \t g:i a') }}</span>
                  </td>
          ...
          <div class="card-footer bg-transparent py-2 d-flex align-items-center justify-content-between border-top">
            ...
            <div class="d-flex align-items-center gap-3">
              <span class="text-muted small">{{ $pages->total() }} items</span>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const STORAGE_KEY = 'cms_pages_screen_options';
        ...
        const match = document.cookie.match(new RegExp('(^| )pages_per_page=([^;]+)'));
        if (match) {
        ...
        applyBtn.addEventListener('click', function() {
            ...
            document.cookie = `pages_per_page=${perPage}; path=/; max-age=31536000`; // 1 year
