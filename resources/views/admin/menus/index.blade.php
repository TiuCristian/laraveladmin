...
<head>
  ...
  <title>Menus | CMS Admin Panel</title>
...
<body>
      ...
      <div class="app-tab-content" style="width: 250px; left: 0;">
        <div class="app-content-inner">
          <div class="tab-content" id="appMenubarTabsContent">
            ...
            <div class="tab-pane fade show " id="mainMenuTab" role="tabpanel" tabindex="0">
                ...
                <ul class="side-menubar">
                  ...
                  <li class="menu-item wp-has-submenu">
                    ...
                    <ul class="wp-submenu wp-submenu-wrap">
                      ...
                      <li class="current"><a href="{{ route('menus.index') }}" class="current">Menus</a></li>
  ...
  <!-- Initialize Sortable for the drag and drop menu -->
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      var el = document.getElementById('menuSortable');
      if (el) {
    ...
    function addItemsToMenu(type) {
      document.querySelectorAll('.' + type + '-checkbox:checked').forEach(function(cb) {
          createMenuItemNode(cb.dataset.title, cb.dataset.url, type);
          cb.checked = false;
      });
    }

    function addCustomLinkToMenu() {
        let url = document.getElementById('customLinkUrl').value;
        let text = document.getElementById('customLinkText').value;
        if (url && text) {
            createMenuItemNode(text, url, 'custom');
            document.getElementById('customLinkUrl').value = 'http://';
    ...
    function createMenuItemNode(title, url, type) {
        let html = `
          <div class="menu-item-bar mb-2" style="max-width: 450px;" data-title="${title}" data-url="${url}" data-type="${type}">
            <div class="menu-item-handle bg-body-tertiary border p-2 d-flex align-items-center justify-content-between">
              <div class="fw-bold text-body small item-title">${title}</div>
              <div class="d-flex align-items-center gap-3">
                <span class="text-muted small item-type" style="text-transform: capitalize;">${type}</span>
                <button class="btn btn-sm btn-link p-0 text-danger shadow-none" type="button" onclick="this.closest('.menu-item-bar').remove()"><i class="fas fa-trash"></i></button>
              </div>
            </div>
          </div>
        `;
        document.getElementById('menuSortable').insertAdjacentHTML('beforeend', html);
    }

    function saveMenu() {
        let items = [];
        document.querySelectorAll('.menu-item-bar').forEach(function(el) {
            items.push({
        ...
        document.getElementById('menuItemsInput').value = JSON.stringify(items);
