	...
	const initSidebarMenu = () => {
		...
		var $activeLink = $('.app-navbar a').filter(function () {
			...
			try {
				...
				// Match exact path, or if current path starts with link path (for things like /admin/users/1/edit matching /admin/users)
				// But we must be careful not to match /admin against /admin/users. 
				// So exact match first:
				if (linkPath === currentPath) return true;
				
				// Optional: if editing, match the parent route if no exact match is found. 
				// This is complex for a simple filter, so let's stick to exact match or basic startsWith for /admin/users
				if (currentPath.startsWith(linkPath + '/') && linkPath !== '/' && linkPath !== '/admin') {
				    return true;
				}
				
				// Fallback for html pages matching (like posts-list.html matching /posts-list.html)
				var linkPage = href.split('/').pop().replace(/\.(php|html|htm)$/i, '');
				var currentPage = currentPath.split('/').pop().replace(/\.(php|html|htm)$/i, '');
				if (linkPage && currentPage && linkPage === currentPage && !href.startsWith('http')) {
				    return true;
				}
		...
		}).last(); // Use last to match the most specific (e.g. /admin/users/create instead of /admin/users)
